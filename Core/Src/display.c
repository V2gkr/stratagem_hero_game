...
void ClearStratagemOnDisplay(void){
  lcd_update_buffer((uint8_t*)"        ", MAX_STRATAGEM_SIZE, 20+STRATAGEM_COL_START);
}
...
void DisplayWaitForStartScreen(void){
  lcd_clear_buffer();
  lcd_update_buffer((uint8_t*)"  Press any button  ",20,0);
  lcd_update_buffer((uint8_t*)"  to start the game ",20,20);
}

void DisplayStartCountDownScreen(uint8_t time_s){
  lcd_clear_buffer();
  lcd_update_buffer((uint8_t*)"  Starting game in  ",20,0);
  uint8_t buf[9];
  snprintf((char*)buf,20,"%d sec ",time_s);
  lcd_update_buffer(buf,6,26);
}

void DisplayActiveGameScreen(uint32_t sequence){
  //lcd_clear_buffer();
  lcd_update_buffer((uint8_t*)"Game started:",13,0);
...
void DisplayAfterRoundInfo(uint8_t time_left,uint16_t points){
  (void)time_left;
  lcd_clear_buffer();
  lcd_update_buffer((uint8_t*)"   Round Complete   ",20,0);
  uint8_t buf[20]={0};
  snprintf((char*)buf,20,"  your score: %03d",points);
  lcd_update_buffer((uint8_t*)buf,strlen((char*)buf),20);
  lcd_update_buffer((uint8_t*)"  Press any button  ",20,40);
  lcd_update_buffer((uint8_t*)"     to continue    ",20,60);
}

void DisplayFinalRoundInfo(uint16_t points){
  lcd_clear_buffer();
  lcd_update_buffer((uint8_t*)"    Game Complete   ",20,0);
  uint8_t buf[20]={0};
  snprintf((char*)buf,20,"  your score: %d",points);
  lcd_update_buffer((uint8_t*)buf,strlen((char*)buf),20);
  lcd_update_buffer((uint8_t*)"  Press any button  ",20,40);
  lcd_update_buffer((uint8_t*)"     to restart     ",20,60);
}

void DisplayInGameTimeout(uint8_t time_s){
  uint8_t buf[7]={0};
  snprintf((char*)buf,7,"%02d  ",time_s);
